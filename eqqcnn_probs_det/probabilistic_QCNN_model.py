...
class Var_ref2(qml.operation.Operation):
    ...
    def __init__(self, params, w):
        ...
        super().__init__()
    def circuit(self):
        Conv3(weights=self.params[0:5], wires=[self.w[0], self.w[1]])
        Pool3(weights=self.params[5:7], wires=[self.w[0], self.w[1]])


'''

class Var_ref3(qml.operation.Operation):
    num_wires = qml.operation.AnyWires
...
# Integrate Var_ref2 into the provided code snippet

'''@qml.qnode(dev, interface="jax")
def probabilistic_qcircuit(X, phi):
    model = PQC3x3(nwires=9)
    model.basic_embedding(X)
    qml.Hadamard(wires=9)
    qml.Hadamard(wires=10)
    for layer in range(layers):
        qml.ctrl(op=model.pqc11, control=(9, 10), control_values=(0, 0))(phi[:11*(layer+1)], w=(0, 1, 2, 3, 4, 5, 6, 7, 8))
        qml.ctrl(op=model.pqc13, control=(9, 10), control_values=(0, 1))(phi[:13*(layer+1)], w=(0, 1, 2, 3, 4, 5, 6, 7, 8))
        qml.ctrl(op=model.pqc15, control=(9, 10), control_values=(1, 0))(phi[:15*(layer+1)], w=(0, 1, 2, 3, 4, 5, 6, 7, 8))
        qml.ctrl(op=model.pqc17, control=(9, 10), control_values=(1, 1))(phi[:17*(layer+1)], w=(0, 1, 2, 3, 4, 5, 6, 7, 8))
...
def Deterministic_Ref_Rot(weights, w):
    ...
    ## Pooling layer
    Pool12(weights[3], wires=[w[0], w[1]])
...
def deterministic_QCNN_model(data, weights):
    ...
    Deterministic_Ref_Rot(weights[8:12], w=[0, 4])
    return qml.probs(op=qml.PauliX(wires=0))


@qml.qnode(device, interface="jax")
def probabilistic_QCNN_model(data, weights):
    feature_map(data)
    Common_Part(weights[:8])
    """
    qml.Hadamard(wires=wires[1])
    ...
    qml.ctrl(qml.Rot, control=8, control_values=0)(weights[0], weights[1], weights[2], wires=[0])
    qml.ctrl(qml.RX, control=8, control_values=0)(weights[3], wires=[4])
    qml.ctrl(qml.IsingXX, control=8, control_values=0)(weights[4], wires=[0,4])
    qml.ControlledQubitUnitary(qml.Hadamard(wires=[1]), control_wires=8, control_values=0)'''
    qml.Hadamard(wires=8)
    qml.ctrl(op=Var_ref, control=8, control_values=0)(weights[8:15], w=[0, 4])
    qml.ctrl(op=Var_rot, control=8, control_values=1)(weights[15:19], w=[0, 4])
    #qml.ControlledQubitUnitary(qml.Hadamard(wires=0), control_wires=8, control_values=1)
    return qml.probs(op=qml.PauliZ(wires=0))  # we have to choose the operator which commutes with PauliX
...
def training(seed: int, model: callable, selected_shape: int, n_epochs: int, batch_size: int) -> list:
    ...
    """
    path = "/home/fv/storage1/qml/file_MNIST_ref_rot_qcnn"
    f = open(path + f'/mnist_ref_rot_{str(seed)}.txt', 'w')
    ...
    for epoch in range(1, n_epochs + 1):
        idxs_dataset = jax.random.choice(key, jnp.array(list(range(new_x_train.shape[0]))), shape=(new_x_train.shape[0],),
                                         replace=False)
        key = jax.random.split(key)[0]
...
test_acc_final = []
model = SelectModel(probabilistic_QCNN_model)
...
axs[0].fill_between(x, np.subtract(loss_final_mean, loss_final_std), np.add(loss_final_mean, loss_final_std),color="lightblue" )
axs[0].plot(x, loss_final_mean, color="blue")
axs[1].set_title("Acc")
axs[1].fill_between(x, np.subtract(acc_final_mean, acc_final_std), np.add(acc_final_mean, acc_final_std),color="tomato" )
axs[1].plot(x, acc_final_mean, color="red")
'''loss, acc, optimal_params = training(seed=12, model=SelectModel(probabilistic_QCNN_model), selected_shape=19,
