...
from data_knowledge import test_loader, train_loader
...
for s, i in enumerate(range(len(weights_loss))):
    print(f"Train quantum LIGHTER model with KNOWLEDGE-DISTILLATION...with soft_target_weight_loss {i+1}")
    train_knowledge_distillation(teacher=nn_deep, student=new_nn_light, train_loader=train_loader, epochs=50,
                                 learning_rate=0.001, Temperature=T_temperatures, soft_target_loss_weight=weights_loss[i], ce_loss_weight=1-weights_loss[i],
